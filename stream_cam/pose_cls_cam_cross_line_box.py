...
def in_box(box, point_test):
    x1 = box[0]
    y1 = box[1]
    x2 = box[2]
    y2 = box[3]
    if (point_test[0]>x1 and point_test[0]<x2) and (point_test[1]>y1 and point_test[1]<y2):
        return True
    else:
        return False
        ...
        if ret:
            ...
            if len(results[0].boxes.data)>0:
                for skeleton in range(len(keypoints_arrs)):
                    ...
                    if (float(results[0].boxes.data[skeleton][4]*100)>humanpose_conf and
                        ((point_position(x_top, x_bottom, left_hand)=='left' or point_position(x_top, x_bottom, right_hand)=='left')) or
                        (in_box(box, left_hand) or in_box(box, right_hand))):
...
#===========================================================================================================#            
            annotated_frame = cv2.line(annotated_frame, np.array([x_bottom, 1080]), np.array([x_top, 0]), (0, 0, 255), 2)  # Tạo bản sao để vẽ đường
            annotated_frame = cv2.rectangle(annotated_frame, (box[0], box[1]), (box[2], box[3]), (0, 0, 255), 2)
