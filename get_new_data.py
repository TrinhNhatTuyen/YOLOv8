...
# {"L0": {"A": [665, 315], "B": [767, 315], "C": [767, 432], "D": [665, 432]}, "L1": {"A": [1240, 330], "B": [1340, 330], "C": [1340, 445], "D": [1240, 445]}}
# points = {
#     'Camtrong':
#     {
#         'A': [1240, 330],
...
cam_name_list = ['CamADat', 'Cayxang', 'VLXD', 'Camtrong']
# cam_name_list = ['Camtrong']
humanpose_conf = 80
queue_len = 10
# ip = '192.168.6.17'
ip = '125.253.117.120'
...
def pose_cls_video(r_queue=False):
    ...
    url = [
            ...
            # 'rtsp://admin:1qazxsw2@kbplawyer.cameraddns.net:5545/cam/realmonitor?channel=1&subtype=0&unicast=true', # Cam ngoài
            'rtsp://admin:1qazxsw2@kbplawyer.cameraddns.net:5549/cam/realmonitor?channel=1&subtype=0&unicast=true', # Cam trong
    ...
    try:
        while True:
            ...
            for CC in range(len(url)):
                ...
                save_img_data = current_time.strftime("%H%M%S_%d%m%Y")
                ...
                if len(results[0].boxes.data)>0:
                    for skeleton in range(len(keypoints_arrs)):
                        ...
                        if (float(results[0].boxes.data[skeleton][4]*100)>humanpose_conf 
                            ...
                            cv2.imwrite(f'img_data/{save_img_data}.jpg', base_img)
                            ...
                            cv2.imwrite(f'img_alert/{save_img_data}.jpg', annotated_frame)
                        else:
                            result_queue(q_lockpicking, False)
                            result_queue(q_climbing, False)
                else:
